...
def split_word(txt):
    ...
    for c in range(co):
        ...
        tokens = tokenslink
    return spans,tokens,pos

def trans_formal(sps,toks,ps,denote):
    index = 0
    label = []
    out = []
    outblank = []
        ...
        while index < len(toks):
            if len(cizu) == 1:
                if sps[index] == label[i][2]:
                    out.append([toks[index], ps[index], 'B-' + label[i][1]])
                    index += 1
                    break
                else:
                    out.append([toks[index], ps[index], 'O'])
                    index += 1
            else:
                if sps[index] == label[i][2]:
                    out.append([toks[index], ps[index], 'B-' + label[i][1]])
                    index += 1
#                    if len(cizu) >= 2:
                    for k in range(len(cizu) - 1):
                        out.append([toks[index], ps[index], 'I-' + label[i][1]])
                        index += 1
...
print('transform json formate to BIO:\tsuccessful!\n^_^\n#########################')
print('done! ^_^')
